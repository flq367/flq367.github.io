...
#申请证书方式1： 
~/.acme.sh/acme.sh  --issue -d 35-212-151-210.nip.io --standalone -k ec-256 --force --insecure
#申请证书方式2： 
...
#安装证书：
~/.acme.sh/acme.sh --install-cert -d 35-212-151-210.nip.io --ecc --key-file /etc/x-ui/server.key --fullchain-file /etc/x-ui/server.crt
```
